...
//-----------------------------
void calculateIndexedNormals(Drawable& drawable) {
    // Clear any existing indexed normals
    drawable.indexedNormals.clear();
    drawable.indexedNormals.resize(drawable.indexedVertices.size(), glm::vec3(0.0f));

    // Temporary storage for face normals
    std::vector<glm::vec3> faceNormals(drawable.indices.size() / 3); 
	std::vector<float> faceAreas(drawable.indices.size() / 3); // for weighted normals

    // Calculate face normals
    for (size_t i = 0; i < drawable.indices.size(); i += 3) {
        unsigned int idx0 = drawable.indices[i];
        unsigned int idx1 = drawable.indices[i + 1];
        unsigned int idx2 = drawable.indices[i + 2];

        glm::vec3 v0 = drawable.indexedVertices[idx0];
        glm::vec3 v1 = drawable.indexedVertices[idx1];
        glm::vec3 v2 = drawable.indexedVertices[idx2];
        ...
        drawable.indexedNormals[idx0] += normal * area;
        drawable.indexedNormals[idx1] += normal * area;
        drawable.indexedNormals[idx2] += normal * area;
    }

    // Normalize the vertex normals
    for (auto& normal : drawable.indexedNormals) {
        normal = glm::normalize(normal);
    }

    if (drawable.indexedNormals.size() != 0) {
        glGenBuffers(1, &drawable.normalsVBO);
        glBindBuffer(GL_ARRAY_BUFFER, drawable.normalsVBO);
        glBufferData(GL_ARRAY_BUFFER, drawable.indexedNormals.size() * sizeof(vec3),
            &drawable.indexedNormals[0], GL_STATIC_DRAW);
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, NULL);
        glEnableVertexAttribArray(1);
    }

    //debug: print portion of the normals
 //   for (size_t i = 0; i < drawable.indexedNormals.size() / 4; i++) {
	//	std::cout << "normal " << i << ": " << drawable.indexedNormals[i].x << " " << drawable.indexedNormals[i].y << " " << drawable.indexedNormals[i].z << std::endl;
	//}

}

void checkTangentsAndBitangents(const vector<vec3>& tangents, const vector<vec3>& bitangents, const vector<vec3>& normals) {
    for (size_t i = 0; i < tangents.size(); ++i) {
        float tangentLength = length(tangents[i]);
        float bitangentLength = length(bitangents[i]);
        float normalLength = length(normals[i]);

        // Check if they are unit vectors
        if (abs(tangentLength - 1.0f) > 0.01f) {
            cout << "Tangent " << i << " is not a unit vector: " << tangentLength << endl;
        }
        if (abs(bitangentLength - 1.0f) > 0.01f) {
            cout << "Bitangent " << i << " is not a unit vector: " << bitangentLength << endl;
        }
        if (abs(normalLength - 1.0f) > 0.01f) {
            cout << "Normal " << i << " is not a unit vector: " << normalLength << endl;
        }

        // Check orthogonality
        if (abs(dot(tangents[i], bitangents[i])) > 0.01f) {
            cout << "Tangent and Bitangent " << i << " are not orthogonal: " << dot(tangents[i], bitangents[i]) << endl;
        }
        if (abs(dot(tangents[i], normals[i])) > 0.01f) {
            cout << "Tangent and Normal " << i << " are not orthogonal: " << dot(tangents[i], normals[i]) << endl;
        }
        if (abs(dot(bitangents[i], normals[i])) > 0.01f) {
            cout << "Bitangent and Normal " << i << " are not orthogonal: " << dot(bitangents[i], normals[i]) << endl;
        }
    }
}

void calculateIndexedTangents(Drawable& drawable) {
    // Clear any existing tangents and bitangents
    drawable.indexedTangents.clear();
    drawable.indexedBitangents.clear();
    drawable.indexedTangents.resize(drawable.indexedVertices.size(), glm::vec3(0.0f));
    drawable.indexedBitangents.resize(drawable.indexedVertices.size(), glm::vec3(0.0f));

    // Temporary storage for face tangents and bitangents
    //std::vector<glm::vec3> faceTangents(drawable.indices.size() / 3);
    //std::vector<glm::vec3> faceBitangents(drawable.indices.size() / 3);
    ...
    for (size_t i = 0; i < drawable.indices.size(); i += 3) {
        ...
        glm::vec2 uv0 = drawable.indexedUVS[idx0];
        glm::vec2 uv1 = drawable.indexedUVS[idx1];
        glm::vec2 uv2 = drawable.indexedUVS[idx2];
        ...
        /*if (f < 1e6) {
            zeroT++;
            zeroB++;
            cout << "f is too small: " << f << endl;
            cout << "This is the result of a degenerate triangle, which is a triangle with zero area." << endl;
            cout << " deltaUVs: " << deltaUV1.x << " " << deltaUV1.y << " " << deltaUV2.x << " " << deltaUV2.y << endl;
            cout << "Their difference:" << deltaUV1.x * deltaUV2.y << " - " << deltaUV2.x * deltaUV1.y << endl;
        }*/

        glm::vec3 tangent = f * (deltaUV2.y * edge1 - deltaUV1.y * edge2);
        glm::vec3 bitangent = f * (-deltaUV2.x * edge1 + deltaUV1.x * edge2);

        //faceTangents[i / 3] = tangent;
        //faceBitangents[i / 3] = bitangent;

        // Accumulate the face tangent and bitangent to each vertex tangent and bitangent
        drawable.indexedTangents[idx0] += tangent;
        drawable.indexedTangents[idx1] += tangent;
        drawable.indexedTangents[idx2] += tangent;

        drawable.indexedBitangents[idx0] += bitangent;
        drawable.indexedBitangents[idx1] += bitangent;
        drawable.indexedBitangents[idx2] += bitangent;
    }

    // Normalize the vertex tangents and bitangents
    for (auto& tangent : drawable.indexedTangents) {
    ...
    for (auto& bitangent : drawable.indexedBitangents) {
		if (glm::length(bitangent) == 0.0f) {
    ...
    if (!drawable.indexedTangents.empty()) {
        glGenBuffers(1, &drawable.tangentsVBO);
        glBindBuffer(GL_ARRAY_BUFFER, drawable.tangentsVBO);
        glBufferData(GL_ARRAY_BUFFER, drawable.indexedTangents.size() * sizeof(glm::vec3),
            &drawable.indexedTangents[0], GL_STATIC_DRAW);
		glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, 0, NULL);
        glEnableVertexAttribArray(3);
    }

    // Upload bitangents to GPU
    if (!drawable.indexedBitangents.empty()) {
        glGenBuffers(1, &drawable.bitangentsVBO);
        glBindBuffer(GL_ARRAY_BUFFER, drawable.bitangentsVBO);
        glBufferData(GL_ARRAY_BUFFER, drawable.indexedBitangents.size() * sizeof(glm::vec3),
            &drawable.indexedBitangents[0], GL_STATIC_DRAW);
        glVertexAttribPointer(4, 3, GL_FLOAT, GL_FALSE, 0, NULL);
        glEnableVertexAttribArray(4);
    }

    // Debug: print portion of the tangents and bitangents
     //for (size_t i = 0; i < drawable.indexedTangents.size() / 4; i++) {
     //    std::cout << "tangent " << i << ": " << drawable.indexedTangents[i].x << " " << drawable.indexedTangents[i].y << " " << drawable.indexedTangents[i].z << std::endl;
     //    std::cout << "bitangent " << i << ": " << drawable.indexedBitangents[i].x << " " << drawable.indexedBitangents[i].y << " " << drawable.indexedBitangents[i].z << std::endl;
     //}

    //checkTangentsAndBitangents(drawable.indexedTangents, drawable.indexedBitangents, drawable.indexedNormals);
